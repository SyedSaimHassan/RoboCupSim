...
std::vector<Eigen::Vector4d> Inv:: getVelocity(){
    ...
    for (int i=0;i<cfg::SystemConfig::numRobots;i++){
        ...
        A << -sin(thetas[0]+angle),cos(angle+thetas[0]),radius,
        ...
        velocities.push_back(velocityToRpm(A*playerVol));
    }
    return velocities;
}
...
Eigen::Vector4d Inv::velocityToRpm(Eigen::Vector4d velocities){
    ...
    for (int i=0;i<velocities.size();i++){
        RpmVel(i)=(velocities(i)*60)/(M_PI*cfg::SystemConfig::wheelDiameter);
    }
    return RpmVel;
}
